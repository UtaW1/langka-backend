defmodule LangkaOrderManagementWeb.ReinstateCategory do
  ...
  def perform(conn, %{"id" => id}) do
    with category when not is_nil(category) <- Product.get_product_category(id),
         {:ok, _} <- Product.reinstate_product_category(category) do
      Plug.Conn.send_resp(conn, 204, "")
    else
      nil ->
        ControllerUtils.render_error(conn, 404, "404.json", "category not found", "")
